...
const Input = (() => {
  ...
  const binds = {
    ...
    x: ['KeyJ'],                     // X — удар ножом
    y: ['KeyK'],                     // Y — способность (позже)
    l: ['KeyQ'],                     // L — подпитка импланта (позже)
    r: ['KeyE'],                     // R — съесть
    start: ['Enter', 'Escape'],
  };
  const now = {}, last = {};
...
const Sfx = (() => {
  ...
  return {
    ...
    heart: () => { tone(60, 0.12, 'sine', 0.25, -20); setTimeout(() => tone(55, 0.14, 'sine', 0.2, -20), 160); },
    coin: () => { tone(1500, 0.06, 'triangle', 0.05); setTimeout(() => tone(2000, 0.1, 'triangle', 0.05), 60); },
    cut: () => { tone(2400, 0.05, 'sawtooth', 0.05, -1800); noise(0.1, 0.06); },
  };
})();
